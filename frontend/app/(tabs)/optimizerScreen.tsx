...
export default function OptimizerScreen(): JSX.Element {
  ...
  const fetchUserRelics = useCallback(async (): Promise<void> => {
    if (!user?.id) { setError('User not authenticated'); setIsLoading(false); return; }
    try {
      setIsLoading(true);
    ...
    } catch (e) { setError('Failed to load relics'); }
    finally { setIsLoading(false); }
  }, [apiUrl, user]);

  useEffect(() => { fetchUserRelics(); }, [fetchUserRelics]);

  // Score and optimization logic
  function optimizeRelics(relics: UserRelic[], sA: string, sB: string) {
    // Simplest stub: pick top by statA + statB across both main & sub
    const scored = relics.map(r => {
      const vA = (r.mainStats[sA] || 0) + (r.subStats[sA] || 0);
      const vB = (r.mainStats[sB] || 0) + (r.subStats[sB] || 0);
      return { relic: r, score: vA + vB };
    });
    const bestSix = scored.sort((a,b) => b.score - a.score).slice(0,6).map(x => x.relic);
    const slots = ['Head','Hands','Body','Feet','Sphere','Link'];
    const result: Record<string, UserRelic> = {};
    slots.forEach((slot, i) => {
      result[slot] = bestSix[i] || bestSix[0];
    });
    return result;
  }
